...
export default function EditBookModal({ modalActive = false, setModalActive, book }) {
  ...
  return (
    <Transition appear show={modalActive} enter="transition duration-100 ease-out"
      ...
      as={Fragment}>
      <Dialog as='div' open={modalActive} onClose={() => { setModalActive(false); reset() }} className={`flex items-center  rounded-3xl z-[150] bg-[rgba(254,136,109,0.3)]  absolute left-[50%] top-[23rem] translate-x-[-50%] translate-y-[-50%] flex-col pb-5 w-[50rem] backdrop-blur-[50px] shadow-lg  add_book_modal ${(isLoading) ? 'pointer-events-none' : ''}`}>
